...
#     controller_node = ControllerSubscriber()

#     while rclpy.ok():
#         controllerData = getControllerValues(controller_node)
#         if controllerData:
#             thrusts = thrustAllocation(controllerData, l, w, theta)
#             thrusts = pct2analog(thrusts)
#             sendData(thrusts, ser_485_left, ser_485_right, ser_PWM)

#     controller_node.destroy_node()
#     rclpy.shutdown()

# 数据发送到电机（已修改为打印数据）
def sendData(thrusts):
    data = []
    for thrust in thrusts:
        direction = thrust > 0
        velocity = abs(thrust)
